    ...
    public static boolean validatePermissions(String[] permissions, Activity activity, int requestCode){
        ...
        if (Build.VERSION.SDK_INT >= 23){
            ...
            for (String permission : permissions) {
                boolean gotPermission = ContextCompat.checkSelfPermission(activity, permission) == PackageManager.PERMISSION_GRANTED;
                if ( !gotPermission) permissionsList.add(permission);
            }
